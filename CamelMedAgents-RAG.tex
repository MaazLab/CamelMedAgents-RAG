...
The query formulation stage transforms structured clinical signals into retrieval-ready representations. Unlike conventional approaches that operate directly on raw user input, this method leverages the structured representation produced by the Medical Parameters Extraction module. This design ensures that retrieval is driven by explicitly defined clinical variables rather than ambiguous natural language.

By decoupling extraction from query generation, the system enforces a controlled data transformation process. This reduces semantic drift, minimizes noise propagation, and enables consistent query construction across heterogeneous medical sources.
\begin{enumerate}
...
\label{fig:camel_methodology_architecture}
\end{figure}
...
\label{fig:query_formulation_pipeline}
\end{figure}
...
\node (A) [box] {Raw Patient Query};
\node (B) [box, below=of A] {Symptom Extraction (LLM)};
\node (C) [box, below=of B] {Temporal Information Extraction (LLM)};
\node (D) [box, below=of C] {Patient Context Extraction (LLM)};
\node (E) [box, below=of D] {Clinical Interpretation (LLM)};
\node (F) [box, below=of E] {Structured Representation (Schema-Validated)};
...
\end{tikzpicture}
\caption{Medical Parameters Extraction Pipeline.}
\label{fig:medical_extraction}
\end{figure}
...
\[
\textbf{(\text{name},\ \text{normalized\_name},\ \text{location},\ \text{severity})}
\]
...
The structured representation produced by the Medical Parameters Extraction module \ref{fig:medical_extraction} serves as the input to the query formulation pipeline \ref{fig:query_formulation_pipeline}.
