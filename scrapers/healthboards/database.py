...
from scrapers.healthboards.config import DB_PATH
...
class Database:
    ...
    async def insert_query_post_mappings_batch(
        self, mappings: list[tuple[int, int, float]]
    ) -> int:
        """Insert multiple (query_id, post_id, similarity_score) mappings in one transaction."""
        if not mappings:
            return 0
        inserted = 0
        try:
            ...
            logger.debug("Batch insert failed for %d mappings, falling back to individual inserts", len(mappings))
            for query_id, post_id, score in mappings:
                result = await self.insert_query_post_mapping(query_id, post_id, score)
