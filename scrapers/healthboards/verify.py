#!/usr/bin/env python3
"""Verification tool: coverage report, duplicate detection, and query mapping stats for healthboards."""
from __future__ import annotations
import argparse
import asyncio

from scrapers.healthboards.config import BASE_URL, EMBEDDING_DIM, SOURCE_NAME
...
async def run_verification(qdrant_url: str | None = None) -> None:
    db = Database()
    await db.connect()
    source_id = await db.get_or_create_source(SOURCE_NAME, BASE_URL)

    print("\n" + "=" * 60)
    print("  HEALTHBOARDS SCRAPER VERIFICATION REPORT")
    print("=" * 60)
    ...
    try:
        ...
        qdrant_count = store.count(source=BASE_URL)
        sqlite_upserted = stats["upserted_to_qdrant"]
        print(f"  Points in Qdrant (source='{BASE_URL}'): {qdrant_count}")
...
def main() -> None:
    setup_logging()
    parser = argparse.ArgumentParser(description="Verify healthboards scraper data coverage and Qdrant sync.")
    parser.add_argument(
        "--qdrant-url",
        type=str,
        default=None,
